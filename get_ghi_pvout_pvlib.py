...
def calculate_performance_ratio(ghi_df, latitude, longitude, sunny_portal_data, pvgis_database='SARAH'):
    ...
    for _, row in ghi_df.iterrows():
        ...
        if sunny_portal_data:
            ...
            # Find the closest key in sunny_portal_data (assuming keys are datetime objects)
            # This assumes sunny_portal_data keys are already sorted or can be efficiently searched
            # A more robust approach might involve converting sunny_portal_data keys to (year, month) tuples
            # For now, direct lookup is used, assuming exact match for month-start datetime
            for sp_date, sp_value in sunny_portal_data.items():
                if sp_date.year == year and sp_date.month == month:
                    real_pvout_value = sp_value
                    break
